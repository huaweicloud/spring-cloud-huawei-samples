 ...
 **/
@RestController
public class ConsumerAz2Controller {
  @Autowired
  private RestTemplate restTemplate;

  private final String az2Info = "provider az2, region: region2; zone: az2";

  /**
   * 同时启动az1、az2的provider服务，开启亲和，返回az2的provider信息
   *
   * @return 是否符合结果
   */
  ...
  public boolean zoneAwareBoth2Az() {
    ...
    return az2Info.equals(result);
  }

  /**
   * 当仅启动az1的provider且未开启强制亲和时，返回az1的provider信息
   *
   * @return 是否符合结果
   */
  ...
  public boolean zoneAwareSingleAz() {
    ...
    String az1Info = "provider az1, region: region1; zone: az1";
    return az1Info.equals(result);
  }

  /**
   * 同时启动az1、az2的provider服务，开启强制亲和(打开application.yaml中的denyCrossZoneLoadBalancing配置后重启服务)，返回az2的provider信息
   *
   * @return 是否符合结果
   */
  @RequestMapping("/zone/denyCross/bothAz")
  public boolean zoneAwareDenyCrossWithProviderAz2() {
    String result = restTemplate.getForObject("http://provider-zone/zone/denyCross", String.class);
    return az2Info.equals(result);
  }

  /**
   * 当仅启动az1的provider且开启强制亲和时(打开application.yaml中的denyCrossZoneLoadBalancing配置后重启服务)，返回错误信息
   *
   * @return 是否符合结果
   */
  @RequestMapping("/zone/denyCross")
  public boolean zoneAwareDenyCrossNoProviderAz2() {
    String errorMessage = "";
    try {
